...
import io.github.prepayments.app.excel.deserializer.AmortizationUpdateExcelFileDeserializer;
...
public class ExcelFileUtils {
    ...
    public static List<AmortizationEntryEVM> deserializeAmortizationFile(byte[] dataEntryFile, String originatingFileToken) {
        ExcelFileDeserializer<AmortizationEntryEVM> excelFileDeserializer = new AmortizationEntryExcelFileDeserializer(
            PoijiOptionsBuilder.settings().ignoreHiddenSheets(true).preferNullOverDefault(true).datePattern(DATETIME_FORMAT).dateTimeFormatter(DateTimeFormatter.ISO_DATE_TIME).build());
    ...
    public static List<AmortizationEntryUpdateEVM> deserializeAmortizationUpdateFile(final byte[] dataEntryFile, final String originatingFileToken) {
        ExcelFileDeserializer<AmortizationEntryUpdateEVM> excelFileDeserializer = new AmortizationUpdateExcelFileDeserializer(
            PoijiOptionsBuilder.settings().ignoreHiddenSheets(true).preferNullOverDefault(true).datePattern(DATETIME_FORMAT).dateTimeFormatter(DateTimeFormatter.ISO_DATE_TIME).build());
        return excelFileDeserializer.deserialize(dataEntryFile)
                                    .stream()
                                    .peek(amortizationUpdate -> amortizationUpdate.setOriginatingFileToken(originatingFileToken))
                                    .collect(ImmutableList.toImmutableList());
    }
}
