...
describe('AmortizationEntry e2e test', () => {
  ...
  it('should create and save AmortizationEntries', async () => {
    ...
    expect(await amortizationEntryUpdatePage.getAmortizationAmountInput()).to.eq(
      '5',
      'Expected amortizationAmount value to be equals to 5'
    );
    expect(await amortizationEntryUpdatePage.getParticularsInput()).to.eq(
      'particulars',
      'Expected Particulars value to be equals to particulars'
    );
    expect(await amortizationEntryUpdatePage.getPrepaymentServiceOutletInput()).to.eq(
      'prepaymentServiceOutlet',
      'Expected PrepaymentServiceOutlet value to be equals to prepaymentServiceOutlet'
    );
    expect(await amortizationEntryUpdatePage.getPrepaymentAccountNumberInput()).to.eq(
      'prepaymentAccountNumber',
      'Expected PrepaymentAccountNumber value to be equals to prepaymentAccountNumber'
    );
    expect(await amortizationEntryUpdatePage.getAmortizationServiceOutletInput()).to.eq(
      'amortizationServiceOutlet',
      'Expected AmortizationServiceOutlet value to be equals to amortizationServiceOutlet'
    );
    expect(await amortizationEntryUpdatePage.getAmortizationAccountNumberInput()).to.eq(
